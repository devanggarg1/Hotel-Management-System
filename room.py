...
class Booking_Room():
    def __init__(self, root):
        ...
        title_lbl = Label(self.root, text="ROOM BOOKING", font=(
            "times new roman", 18, "bold"), bg="black", fg="gold", bd=3, relief=RIDGE)
        ...
        lblcusPhone = Label(DataFrameLeft, font=(
            "arial", 12, "bold"), text="Customer Phone No", padx=2, pady=6)
        lblcusPhone.grid(row=0, column=0, sticky=W)
        cust_Phone = ttk.Entry(DataFrameLeft, textvariable=self.var_mobile, font=(
            "arial", 13, "bold"), width=20)
        cust_Phone.grid(row=0, column=1, sticky=W)

        # ============btn====================
        btnfetch = Button(DataFrameLeft, command=self.cont_fetch, text="Fetch Data", font=(
            "arial", 9, "bold"), width=8, bg="black", fg="gold")
        btnfetch.place(x=350, y=4)

        c_check_inDate = Label(DataFrameLeft, font=(
            "arial", 12, "bold"), text="Check-in Date:", padx=2, pady=6)
        c_check_inDate.grid(row=1, column=0, sticky=W)
        txtc_check_inDate = ttk.Entry(
            DataFrameLeft, textvariable=self.var_checkin, font=("arial", 13, "bold"), width=29)
        txtc_check_inDate.grid(row=1, column=1)

        lblCheckOutdate = Label(DataFrameLeft, font=(
            "arial", 12, "bold"), text="Check-out Date:", padx=2, pady=6)
        lblCheckOutdate.grid(row=2, column=0, sticky=W)
        txtCheckOutdate = ttk.Entry(DataFrameLeft, textvariable=self.var_checkout, font=(
            "arial", 13, "bold"), width=29)
        txtCheckOutdate.grid(row=2, column=1)

        label_RoomType = Label(DataFrameLeft, font=(
        ...
        lblAvailableRoom = Label(DataFrameLeft, font=(
        ...
        lblMeal = Label(DataFrameLeft, font=(
        ...
        lblNoOfDays = Label(DataFrameLeft, font=(
            "arial", 12, "bold"), text="No Of Days:", padx=2, pady=6)
        lblNoOfDays.grid(row=6, column=0, sticky=W)
        txtNoOfDays = ttk.Entry(DataFrameLeft, textvariable=self.var_noofdays, font=(
            "arial", 13, "bold"), width=29)
        txtNoOfDays.grid(row=6, column=1)

        lblPaidtax = Label(DataFrameLeft, font=(
            "arial", 12, "bold"), text="Paid Tax:", padx=2, pady=6)
        lblPaidtax.grid(row=7, column=0, sticky=W)
        txtPaidtax = ttk.Entry(DataFrameLeft, textvariable=self.var_paidtax, font=(
            "arial", 13, "bold"), width=29)
        txtPaidtax.grid(row=7, column=1)

        lblsubtotal = Label(DataFrameLeft, font=(
            "arial", 12, "bold"), text="Actual Total:", padx=2, pady=6)
        lblsubtotal.grid(row=8, column=0, sticky=W)
        txtsubtotal = ttk.Entry(DataFrameLeft, textvariable=self.var_actualtotal, font=(
            "arial", 13, "bold"), width=29)
        txtsubtotal.grid(row=8, column=1)

        lblTotalCost = Label(DataFrameLeft, font=(
            "arial", 12, "bold"), text="Total Cost:", padx=2, pady=6)
        lblTotalCost.grid(row=9, column=0, sticky=W)
        txtTotalCost = ttk.Entry(DataFrameLeft, textvariable=self.var_total, font=(
            "arial", 13, "bold"), width=29)
        txtTotalCost.grid(row=9, column=1)
        # ==========bill btn================
        btnBill = Button(DataFrameLeft, text="Bill", command=self.total, font=(
            "arial", 11, "bold"), width=10, bg="black", fg="gold")
        ...
        # ===================================ButtonFrame=====================================
        btnAddData = Button(ButtonFrame1, text="Save", command=self.add_data, font=(
            "arial", 11, "bold"), width=10, bg="black", fg="gold")
        btnAddData.grid(row=0, column=0, padx=1)

        btnShowData = Button(ButtonFrame1, text="Update", command=self.update_data, font=(
            "arial", 11, "bold"), width=10, bg="black", fg="gold")
        btnShowData.grid(row=0, column=1, padx=1)

        btnUpdate = Button(ButtonFrame1, text="Delete", command=self.roomDelete, font=(
            "arial", 11, "bold"), width=10, bg="black", fg="gold")
        btnUpdate.grid(row=0, column=2, padx=1)

        btnDelete = Button(ButtonFrame1, text="Reset", command=self.reset, font=(
            "arial", 11, "bold"), width=10, bg="black", fg="gold")
        btnDelete.grid(row=0, column=3, padx=1)

        # =======table&search=====================================================================================
        Table_frame = LabelFrame(self.root, text="View Customer Details & Search System", font=(
            "arial", 11, "bold"), bd=3, relief=RIDGE)
        Table_frame.place(x=455, y=240, width=845, height=70)

        # ==========Search By========
        lblSearch = Label(Table_frame, font=("arial", 15, "bold"),
        ...
        txtSearch = ttk.Entry(
            Table_frame, width=12, textvariable=self.serchTxt_var, font=("times new roman", 15))
        ...
        btnExit = Button(Table_frame, text="SHOW ALL", command=self.fatch_data, font=(
    ...
    def add_data(self):
        if self.var_mobile.get() == "" or self.var_checkin.get() == "":
            ...
            try:
                ...
                messagebox.showerror(
                    "Error", f" This Room has been Already Booked :{str(es)}", parent=self.root)
    ...
    def search_data(self):
        ...
        my_cursor.execute("select * from room where " + str(self.serch_var.get()
                                                            )+" LIKE '%"+str(self.serchTxt_var.get())+"%'")
        rows = my_cursor.fetchall()
        if len(rows) != 0:
            self.room_table.delete(*self.room_table.get_children())
            for i in rows:
    ...
    def cont_fetch(self):
        if self.var_mobile.get() == "":
            messagebox.showerror(
                "Error", "Plaese Enter Phone Number", parent=self.root)
        else:
            conn = mysql.connector.connect(
                host="localhost", user="root", password="root", database="mydata")
            my_cursor = conn.cursor()
            ...
            # print(row)
                ...
                lblname = Label(showDframe, text="Name:",
                                font=("arial", 12, "bold"))
                ...
                lbladdress = Label(showDframe, text="Gender:",
                                   font=("arial", 12, "bold"))
                ...
                lbladdress = Label(showDframe, text="Email:",
                                   font=("arial", 12, "bold"))
                ...
                lbladdress = Label(showDframe, text="Address:",
                                   font=("arial", 12, "bold"))
                ...
                lbladdress = Label(showDframe, text="Contact:",
                                   font=("arial", 12, "bold"))
