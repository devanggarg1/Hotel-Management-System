...
class DetailsAdd:
    def __init__(self, root):
        ...
        title_lbl = Label(self.root, text="Room Adding depatment", font=(
        ...
        # ================labels & entry=================
        l_floor = Label(DataFrameRight, text="Floor",
                        fg="black", font=("times new roman", 15))
        l_floor.place(x=0, y=80)

        self.floor_add_var = StringVar()
        entry_Floor = ttk.Entry(
            DataFrameRight, textvariable=self.floor_add_var, width=15, font=("times new roman", 15))
        entry_Floor.place(x=135, y=80)

        l_roomNo = Label(DataFrameRight, text="Room No",
                         fg="black", font=("times new roman", 15))
        l_roomNo.place(x=0, y=110)

        self.room_add_var = StringVar()
        entry_Medicine = ttk.Entry(
            DataFrameRight, textvariable=self.room_add_var, width=15, font=("times new roman", 15))
        entry_Medicine.place(x=135, y=110)

        l_roomType = Label(DataFrameRight, text="Room Type",
                           fg="black", font=("times new roman", 15))
        l_roomType.place(x=0, y=140)

        self.type_room = StringVar()
        roomType_combo = ttk.Combobox(DataFrameRight, width=13, textvariable=self.type_room, font=(
            "times new roman", 15), state="readonly")
        roomType_combo['values'] = (
            "Select Option", "Single", "Double", "Laxary")
        roomType_combo.place(x=135, y=140)
        roomType_combo.current(0)

        # ================================ right side frame table ==============================================
        side_frame = LabelFrame(self.root, bd=4, text="Show Room Deatails",
                                relief=RIDGE, bg="white", font=("times new roman", 15))
        ...
        self.room_table = ttk.Treeview(side_frame, column=(
            "floor", "roomno", "type"), xscrollcommand=sc_x.set, yscrollcommand=sc_y.set)
        ...
        add_btn = Button(down_frame, text="ADD", command=self.add_room, font=(
            "arial", 12, "bold"), width=12, fg="gold", bg="black")
        add_btn.grid(row=0, column=0, pady=2)

        update_btn = Button(down_frame, text="UPDATE", command=self.room_update, font=(
            "arial", 12, "bold"), width=12, fg="gold", bg="black")
        update_btn.grid(row=1, column=0, pady=2)

        delete_btn = Button(down_frame, text="DELETE", command=self.roomDelete, font=(
            "arial", 12, "bold"), width=12, fg="gold", bg="black")
        delete_btn.grid(row=2, column=0, pady=2)

        clear_btn = Button(down_frame, text="CLEAR", command=self.clear_room, font=(
            "arial", 12, "bold"), width=12, fg="gold", bg="black")
    ...
    def add_room(self):
        if self.floor_add_var.get() == "" or self.room_add_var.get() == "":
            messagebox.showerror(
                "Error", "Please enter Floor", parent=self.root)
        else:
